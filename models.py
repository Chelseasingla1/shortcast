...
from sqlalchemy import String, Enum, BigInteger, Time, DateTime, ForeignKey, SmallInteger
...
class User(db.Model,UserMixin):
    ...
    created_at:Mapped[DateTime] = mapped_column(DateTime,func.now(),nullable=False)
...
class Podcast(db.Model):
    ...
    publish_date:Mapped[DateTime] = mapped_column(DateTime,default=func.now(),nullable=False)
...
class Episode(db.Model):
    ...
    podcast_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('podcast.id'),nullable=False)


class Subscription(db.Model):
    user_id:Mapped[int] = mapped_column(BigInteger,ForeignKey('user.id'),primary_key=True)
    podcast_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('podcast.id'),primary_key=True)
    subscribed_date:Mapped[DateTime] = mapped_column(DateTime,default=func.now(),nullable=False)
...
class Favourite(db.Model):
    user_id:Mapped[int] = mapped_column(BigInteger,ForeignKey('user.id'),nullable=False,primary_key=True)
    podcast_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('podcast.id'),primary_key=True,nullable=True)
    episode_id:Mapped[int] = mapped_column(BigInteger,ForeignKey('episode.id'),primary_key=True,nullable=True)
    added_date:Mapped[DateTime] = mapped_column(DateTime,default=func.now(),nullable=False)
...
class Rating(db.Model):
    ...
    review_date:Mapped[DateTime] = mapped_column(DateTime,default=func.now(),nullable=False)
...
class Download(db.Model):
    ...
    podcast_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('podcast.id'), primary_key=True,nullable=True)
    episode_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('episode.id'), primary_key=True, nullable=True)
    download_date: Mapped[DateTime] = mapped_column(DateTime, default=func.now(), nullable=False)
...
class PlaylistItem(db.Model):
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True)
    podcast_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('podcast.id'), nullable=True)
    episode_id: Mapped[int] = mapped_column(BigInteger,ForeignKey('episode.id'),nullable=True)
    added_date: Mapped[DateTime] = mapped_column(DateTime, func.now(), nullable=False)
    podcast = db.relationship('Podcast', backref='playlist_item',uselist=False)
    episodes = db.relationship('Episode', backref='rating',uselist=False)
...
class SharedPlaylist(db.Model):
    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('user.id'), nullable=False, primary_key=True)
    playlist_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('playlist.id'), primary_key=True, nullable=False)
...
class PlaylistPlaylistitem(db.Model):
    playlist_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('playlist.id'), primary_key=True, nullable=False)
    playlist_item_id: Mapped[int] = mapped_column(BigInteger, ForeignKey('playlistitem.id'), primary_key=True, nullable=False)
